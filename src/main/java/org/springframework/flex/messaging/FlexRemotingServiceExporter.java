...
public class FlexRemotingServiceExporter implements FlexFactory, InitializingBean, DisposableBean, BeanNameAware {
	...
	public void afterPropertiesSet() throws Exception {
		...
		RemotingDestination destination = (RemotingDestination) remotingService.createDestination(getServiceId());
		destination.setFactory(this);
		destination.start();
		
		Assert.isInstanceOf(JavaAdapter.class, destination.getAdapter(), "Spring beans exported as a RemotingDestination require a JavaAdapter.");
	...
	private List<RemotingMethod> getRemotingMethods(String[] methodNames) {
		...
		for (int i=0; i<methodNames.length; i++) {
			...
			Assert.isTrue(ClassUtils.hasAtLeastOneMethodWithName(service.getClass(), name), "Could not find method with name '"+name+"' on the exported service of type "+service.getClass());
